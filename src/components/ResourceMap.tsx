  ...
  return (
    <div
      ...
      <Handle
        ...
        className="!w-3 !h-3 !-top-1.5 !bg-bia-text-secondary"
      />
      <Handle
        type="source"
        position={Position.Bottom}
        className="!w-3 !h-3 !-bottom-1.5 !bg-bia-text-secondary"
      />
      <Handle
        type="target"
        position={Position.Left}
        id="l"
        className="!w-3 !h-3 !-left-1.5 !bg-bia-text-secondary"
      />
      <Handle
        type="source"
        position={Position.Right}
        id="r"
        className="!w-3 !h-3 !-right-1.5 !bg-bia-text-secondary"
      />
      ...
      <div className="flex items-center gap-3 mb-2">
        ...
        <div className="flex-1 min-w-0">
          <div
            className="text-sm font-bold text-bia-text-primary truncate"
            title={data.label}>
            {data.label}
          </div>
          <div
            ...
            style={{ color }}>
            {data.resourceTypeLabel}
          </div>
        </div>
        {(rtoViolation || rpoViolation) && (
      ...
      <div className="flex gap-2">
        {data.rto && (
          <div
            className={`px-1.5 py-0.5 rounded text-[10px] font-mono border ${rtoViolation ? "bg-red-500/20 text-red-200 border-red-500/50" : "bg-white/10 text-bia-text-secondary border-white/5"}`}>
            RTO:{" "}
            <span
              className={rtoViolation ? "text-white font-bold" : "text-white"}>
              {data.rto.value}
              {data.rto.unit.charAt(0)}
            </span>
          </div>
        )}
        {data.rpo && (
          <div
            className={`px-1.5 py-0.5 rounded text-[10px] font-mono border ${rpoViolation ? "bg-red-500/20 text-red-200 border-red-500/50" : "bg-white/10 text-bia-text-secondary border-white/5"}`}>
            RPO:{" "}
            <span
              className={rpoViolation ? "text-white font-bold" : "text-white"}>
              {data.rpo.value}
              {data.rpo.unit.charAt(0)}
            </span>
          </div>
        )}
      </div>
...
function ProcessNode({ data, selected }: NodeProps<CustomProcessNode>) {
  ...
  const rtoViolation =
    !data.isMain &&
    data.mainRto !== undefined &&
    data.rto !== undefined &&
    data.rto > data.mainRto;
  const rpoViolation =
    !data.isMain &&
    data.mainRpo !== undefined &&
    data.rpo !== undefined &&
    data.rpo > data.mainRpo;
  ...
  return (
    <div
      ...
      <div className="flex gap-2">
        {data.rto !== undefined && (
          <div
            className={`px-1.5 py-0.5 rounded text-[10px] font-mono border ${rtoViolation ? "bg-red-500/20 text-red-200 border-red-500/50" : "bg-blue-500/10 text-blue-200 border-blue-500/20"}`}>
            RTO:{" "}
            <span
              className={rtoViolation ? "text-white font-bold" : "text-white"}>
              {data.rto}h
        ...
        {data.rpo !== undefined && (
          <div
            className={`px-1.5 py-0.5 rounded text-[10px] font-mono border ${rpoViolation ? "bg-red-500/20 text-red-200 border-red-500/50" : "bg-blue-500/10 text-blue-200 border-blue-500/20"}`}>
            RPO:{" "}
            <span
              className={rpoViolation ? "text-white font-bold" : "text-white"}>
              {data.rpo}h
...
function DraggableLibrary() {
  ...
  const [activeTab, setActiveTab] = useState<"resources" | "processes">(
    "resources",
  );
  ...
  return (
    <aside className="w-80 border-r border-white/10 bg-bia-glass flex flex-col h-full">
      <div className="p-4 border-b border-white/10">
        <h2 className="text-sm font-semibold text-bia-text-secondary uppercase tracking-wider mb-3">
          Library
        </h2>
        ...
        {activeTab === "resources"
          ? filteredResources.map((res) => (
              <div
                ...
                draggable
                onDragStart={(event) =>
                  onDragStart(event, "resourceNode", {
                    id: res.id,
                    label: res.name,
                ...
                <div className="flex items-center gap-3">
                  <div
                    ...
                    style={{
                      backgroundColor: RESOURCE_COLORS[res.type],
                    }}></div>
                  <span className="text-sm text-bia-text-primary group-hover:text-white truncate">
                    {res.name}
                  </span>
                </div>
              </div>
...
function DependencyTable() {
  const { processes, dependencies, removeDependency, getProcessById } =
    useStore();
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-bia-border">
              {dependencies.map((dep) => (
                <tr
                  key={dep.id}
                  className="hover:bg-bia-glass-hover transition-colors">
                  <td className="p-3 text-bia-text-primary font-medium">
                    {getProcessName(dep.sourceProcessId)}
                  </td>
                  ...
                  <td className="p-3">
                    <span
                      className={`px-2 py-1 rounded text-xs border border-white/5 bg-white/5 ${DEP_COLORS[dep.type]}`}>
                      {dep.type}
                    </span>
                  </td>
                  <td className="p-3 text-bia-text-primary">
                  ...
                  <td className="p-3 text-bia-text-secondary text-sm">
                    {dep.description || "-"}
                  </td>
                  <td className="p-3">
                    <button
              ...
              {dependencies.length === 0 && (
                <tr>
                  <td
                    colSpan={6}
                    className="p-8 text-center text-bia-text-tertiary">
...
function ResourceMapCanvas() {
  ...
  const [pendingDeletions, setPendingDeletions] = useState<Map<string, string>>(
    new Map(),
  ); // ID -> Type
  const [upstreamPickerOpen, setUpstreamPickerOpen] = useState(false);
  const [upstreamSearch, setUpstreamSearch] = useState("");

  const filteredUpstreamProcesses = processes.filter(
    (p) =>
      p.id !== selectedProcessId &&
      p.name.toLowerCase().includes(upstreamSearch.toLowerCase()) &&
      !dependencies.some(
        (d) =>
          d.sourceProcessId === p.id && d.targetProcessId === selectedProcessId,
      ), // Exclude existing upstream
  );
  ...
  const saveLayout = useCallback(async () => {
    ...
    if (pendingDeletions.size > 0) {
      ...
      pendingDeletions.forEach((type, id) => {
        if (type === "process-link")
          promises.push(unlinkResourceFromProcess(id));
        else if (type === "resource-dep")
          promises.push(removeResourceDependency(id));
  ...
  useEffect(() => {
    ...
    if (proc) {
      ...
      initialNodes.push({
        id: proc.id,
        type: "processNode",
        position: savedLayout?.nodes?.find((n: any) => n.id === proc.id)
          ?.position || { x: 400, y: 50 },
        data: {
          label: proc.name,
          criticality: proc.criticality,
    ...
    if (savedLayout && savedLayout.nodes) {
      savedLayout.nodes.forEach((savedNode: any) => {
        if (savedNode.id !== selectedProcessId) {
          // Skip main, already added
      ...
      processResourceLinks
        .filter(
          (link) =>
            link.processId === selectedProcessId &&
            !pendingDeletions.has(link.id),
            ...
            if (res) {
              initialNodes.push({
                ...
                data: {
                  ...
                  resourceTypeLabel:
                    (SAFE_RESOURCE_TYPES &&
                      SAFE_RESOURCE_TYPES.find((t) => t && t.value === res.type)
                        ?.label) ||
                    res.type,
                  rto: res.rto,
                  rpo: res.rpo,
  ...
  useEffect(() => {
    ...
    processResourceLinks.forEach((link) => {
      ...
      if (
        visibleNodeIds.has(link.processId) &&
        visibleNodeIds.has(link.resourceId)
      ) {
    ...
    resourceDependencies.forEach((dep) => {
      ...
      if (
        visibleNodeIds.has(dep.sourceResourceId) &&
        visibleNodeIds.has(dep.targetResourceId)
      ) {
        ...
        newEdges.push({
          ...
          style: {
            stroke: strokeColor,
            strokeWidth: dep.isBlocking ? 3 : 1.5,
            opacity: 0.8,
          },
    ...
    dependencies.forEach((dep) => {
      ...
      if (
        visibleNodeIds.has(dep.sourceProcessId) &&
        visibleNodeIds.has(dep.targetProcessId)
      ) {
        newEdges.push({
          id: dep.id,
  ...
  const onConnect = useCallback(
    (params: Connection) => {
      ...
      // Determine types
      const isSourceResource = businessResources.some(
        (r) => r.id === params.source,
      );
      const isTargetResource = businessResources.some(
        (r) => r.id === params.target,
      );
              ...
              {upstreamPickerOpen && (
                <div className="absolute top-full right-0 mt-2 w-72 bg-bia-card border border-bia-border rounded-lg shadow-xl p-3 z-50 flex flex-col gap-2 max-h-96">
                  <div className="flex items-center justify-between pb-2 border-b border-white/10">
                    <span className="text-xs font-semibold text-bia-text-secondary uppercase">
                      Select Process
                    </span>
                    <button onClick={() => setUpstreamPickerOpen(false)}>
                      <X className="w-4 h-4 text-bia-text-tertiary hover:text-white" />
                    </button>
                  </div>
                  <input
                    autoFocus
                  ...
                  <div className="flex-1 overflow-y-auto space-y-1 min-h-[100px]">
                    ...
                    {filteredUpstreamProcesses.length === 0 && (
                      <div className="text-center py-4 text-xs text-bia-text-tertiary">
...
export function ResourceMap() {
  ...
  useEffect(() => {
    ...
    // 1. Check Resources
    processResourceLinks
      .filter((l) => l.processId === selectedProcessId)
      .forEach((link) => {
        const res = businessResources.find((r) => r.id === link.resourceId);
    ...
    const upstream = dependencies.filter(
      (d) => d.targetProcessId === selectedProcessId,
    );
      ...
      <div className="animate-fade-in space-y-6">
        <div>
          ...
          <p className="text-bia-text-secondary mt-1">
            Select a business process to visualize and map its dependencies.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {processes.map((p) => (
            <button
              ...
              <div className="flex justify-between items-start mb-2">
                <h3 className="text-lg font-semibold text-bia-text-primary group-hover:text-pink-400 transition-colors">
                  {p.name}
                </h3>
              </div>
          ...
          {processes.length === 0 && (
            <div className="col-span-full text-center p-12 glass-panel border-dashed border-2 border-white/10">
              ...
              <p className="text-bia-text-tertiary mt-2">
                Go to the Process Registry to create your first process.
              </p>
            </div>
          )}
        </div>
      ...
      <div className="flex justify-between items-center mb-4 shrink-0">
        ...
        <div className="flex items-center gap-4">
          ...
          {spofProcesses.length > 0 && (
            <button
              ...
              <span className="font-semibold">
                {" "}
                {spofProcesses.length} Bottlenecks
              </span>
            </button>
          )}
          ...
          {complianceViolations.length > 0 && (
            <button
              onClick={() => {
                setWarningModalType("compliance");
                setWarningModalOpen(true);
              }}
              ...
              <span className="font-semibold">
                {" "}
                {complianceViolations.length} Compliance Gaps
              </span>
            </button>
          )}
