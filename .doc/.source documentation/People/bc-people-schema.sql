...
CREATE TABLE bc_roles (
    ...
    description TEXT,
    role_type ENUM('executive', 'strategic', 'operational', 'support', 'specialist') NOT NULL,
    criticality_level ENUM('critical', 'high', 'medium', 'low') DEFAULT 'medium',
...
CREATE TABLE bc_team_structure (
    ...
    name VARCHAR(100) NOT NULL,
    description TEXT,
    structure_type ENUM('crisis_team', 'recovery_team', 'emergency_response', 'communication_team', 'business_unit') NOT NULL,
...
CREATE TABLE bc_role_assignments (
    ...
    assignment_type ENUM('primary', 'backup', 'alternate', 'deputy') DEFAULT 'primary',
    start_date DATE NOT NULL DEFAULT CURRENT_DATE,
    end_date DATE,
    is_active BOOLEAN DEFAULT TRUE,
    notification_preferences JSONB, -- Email, SMS, phone preferences
    availability_status ENUM('available', 'on_leave', 'unavailable', 'limited') DEFAULT 'available',
...
CREATE TABLE bc_contact_methods (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    person_id UUID NOT NULL REFERENCES bc_people(id) ON DELETE CASCADE,
    contact_type ENUM('email', 'phone', 'mobile', 'satellite_phone', 'radio', 'pager', 'teams', 'slack', 'whatsapp') NOT NULL,
...
CREATE TABLE bc_training_records (
    ...
    status ENUM('completed', 'in_progress', 'expired', 'failed') NOT NULL,
...
CREATE TABLE bc_competencies (
    ...
    name VARCHAR(100) NOT NULL,
    description TEXT,
    competency_category ENUM('technical', 'leadership', 'communication', 'crisis_management', 'regulatory', 'business_specific') NOT NULL,
...
CREATE TABLE bc_person_competencies (
    ...
    assessment_method ENUM('self_assessment', 'manager_assessment', 'peer_review', 'formal_test', 'observation') NOT NULL,
    evidence TEXT,
...
CREATE TABLE bc_succession_plans (
    ...
    readiness_level ENUM('ready_now', 'ready_in_6_months', 'ready_in_1_year', 'development_needed') NOT NULL,
...
CREATE TABLE bc_communication_cascades (
    ...
    cascade_type ENUM('emergency', 'business_disruption', 'recovery', 'all_clear', 'test') NOT NULL,
